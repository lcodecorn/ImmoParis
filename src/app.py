...
def load_and_preprocess_data():
    ...
    # Transaction features (must exist BEFORE train/test split)
    # Keep the same logic/semantics as `src/run.py`.
...
def load_models():
    ...
    # Load model
    try:
        with open(os.path.join(MODEL_DIR, "model_cat.pkl"), "rb") as f:
            models['catboost'] = {
                'model': pickle.load(f),
                'name': 'CatBoost Regressor',
                'available': True
            }
        with open(os.path.join(MODEL_DIR, "preprocessor_cat.pkl"), "rb") as f:
            models['catboost']['preprocessor'] = pickle.load(f)
    except FileNotFoundError:
        models['catboost'] = {'available': False, 'name': 'CatBoost Regressor'}
    
    try:
        with open(os.path.join(MODEL_DIR, "model_random_forest.pkl"), "rb") as f:
            models['random_forest'] = {
                'model': pickle.load(f),
                'name': 'Random Forest Regressor (sklearn)',
                'available': True
            }
        with open(os.path.join(MODEL_DIR, "preprocessor_random_forest.pkl"), "rb") as f:
            models['random_forest']['preprocessor'] = pickle.load(f)
    except FileNotFoundError:
        models['random_forest'] = {'available': False, 'name': 'Random Forest Regressor (sklearn)'}
    
    try:
        with open(os.path.join(MODEL_DIR, "model_numpy.pkl"), "rb") as f:
            models['numpy'] = {
                'model': pickle.load(f),
                'name': 'Random Forest Regressor (Pure NumPy)',
                'available': True
            }
        with open(os.path.join(MODEL_DIR, "preprocessor_numpy.pkl"), "rb") as f:
...
try:
    ...
    if not available_models:
        st.error("❌ No models found! Please ensure model files are in the 'Model' folder.")
        st.stop()
    ...
    **Available Models:**
    ...
    - `Data/xp.csv`
    - `Data/metro.csv`
    - `Model/model_cat.pkl` and `Model/preprocessor_cat.pkl` (for CatBoost)
    - `Model/model_random_forest.pkl` and `Model/preprocessor_random_forest.pkl` (for Random Forest)
    - `Model/model_numpy.pkl` and `Model/preprocessor_numpy.pkl` (for NumPy Random Forest)
